...
# Dictionary to store proficiency ratings
proficiency_ratings = {}

st.header("Skills Assessment")

# Function to create rating buttons
def create_rating_buttons(skill, ratings_dict):
    cols = st.columns(5)
    for i, col in enumerate(cols, start=1):
        if col.button(str(i), key=f"{skill}_{i}"):
            ratings_dict[skill] = i
    ...
    for skill, col in zip(row, cols):
        with col:
            if st.checkbox(skill, key=f"checkbox_{skill}"):
                ...
                create_rating_buttons(skill, proficiency_ratings)
...
def add_custom_skill():
    ...
    if custom_skill and custom_skill not in custom_skills:
        ...
        create_rating_buttons(custom_skill, custom_skill_ratings)
...
if st.button("Submit"):
    ...
    if not first_name or not last_name:
    ...
    elif not proficiency_ratings and not custom_skill_ratings:
        ...
        all_skills = {**proficiency_ratings, **custom_skill_ratings}
